...
prs=$(curl -s \
  ...
  https://api.github.com/repos/iTwin/appui/pulls)
...
masterCommit=$(curl -s \
  -H "Accept: application/vnd.github.v3+json" \
  -H "Authorization: token $oauth" \
  https://api.github.com/repos/iTwin/appui/commits/master)
...
for pr in $(echo "${prs}" | jq -r '.[] | @base64'); do
  ...
  then
    refCommit=$(curl -s \
              -H "Accept: application/vnd.github.v3+json" \
              -H "Authorization: token $oauth" \
              https://api.github.com/repos/iTwin/appui/commits/${ref})
  ...
  branchLatest=$(curl -s \
              -H "Accept: application/vnd.github.v3+json" \
              -H "Authorization: token $oauth" \
              https://api.github.com/repos/iTwin/appui/branches/${branch})
  ...
  #   e.g. https://api.github.com/repos/iTwin/appui/statuses/c29a168b6a201052098ea6743041868bfbadaa4f
  statuses=$(curl -s \
              -H "Accept: application/vnd.github.v3+json" \
              -H "Authorization: token $oauth" \
              $(_jq ${pr} '.statuses_url'))
  ...
  for status in $(echo "${statuses}" | jq -r '.[] | @base64'); do
    ...
    updateUrl=https://api.github.com/repos/iTwin/appui/statuses/$(_jq ${pr} '.head.sha')
