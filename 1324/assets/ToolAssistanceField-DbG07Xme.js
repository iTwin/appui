import { L as Logger, R as RelativePosition, B as Button } from "./Dialog-DTkGrnAC.js";
import { bT as useDraggedItem, U as UiFramework, bU as Timer, bV as useCursorInformationStore, bW as CursorPopupManager, bX as CursorInformation, bY as Point, J as Icon, a8 as Text, bZ as Dialog, b7 as TitleBar, I as IModelApp, b_ as UiStateStorageContext, H as useTranslation, b$ as UiStateStorageStatus, M as MessageManager, c0 as ProcessDetector, b6 as StatusBarPopover, c1 as StatusBarDialog, b1 as SvgClose, c2 as SvgPin, c3 as ToggleSwitch, c4 as ToolAssistanceInputMethod, h as ToolAssistanceImage, c5 as FillCentered, aK as SvgAdd } from "./appui-react-D5ydBozH.js";
import { c as cx } from "./SvgCloseSmall-Bu3Oy4Gr.js";
import { r as reactExports } from "./index-DVOlmhHI.js";
import { T as Tabs } from "./Tabs-D-wnJwh7.js";
...
function useCursorPrompt(args) {
  ...
  const open = reactExports.useCallback(() => {
    ...
    setIsOpen(true);
    (_a = timerRef.current) == null ? void 0 : _a.start();
  }, []);
  return { open };
}
...
class ToolAssistanceDialog extends reactExports.PureComponent {
  render() {
    const className = cx("nz-footer-toolAssistance-dialog", this.props.className);
    return reactExports.createElement(
      Dialog,
...
class ToolAssistanceItem extends reactExports.PureComponent {
  render() {
    const className = cx("nz-footer-toolAssistance-item", this.props.className);
...
class NewDot extends reactExports.PureComponent {
  render() {
    const className = cx("nz-footer-toolAssistance-newDot", this.props.className);
...
class ToolAssistanceInstruction extends reactExports.PureComponent {
  render() {
    const className = cx("nz-footer-toolAssistance-instruction", this.props.className);
...
class ToolAssistanceSeparator extends reactExports.PureComponent {
  render() {
    const className = cx("nz-footer-toolAssistance-separator", this.props.className);
    return reactExports.createElement(
      "div",
      { className, style: this.props.style },
...
function ToolAssistanceField(props) {
  var _a, _b, _c;
  const { includePromptAtCursor = true, cursorPromptTimeout = 5e3, fadeOutCursorPrompt = true, defaultPromptAtCursor = false, uiStateStorage: uiStateStorageProp, promptAtContent = false } = props;
  ...
  const mainInstruction = (_a = state.instructions) == null ? void 0 : _a.mainInstruction.text;
  ...
  const hasMouseInstructions = !!((_b = instructions == null ? void 0 : instructions.sections) == null ? void 0 : _b.some((section) => {
    return section.instructions.some((instruction) => isMouseInstruction(instruction));
  }));
  const hasTouchInstructions = !!((_c = instructions == null ? void 0 : instructions.sections) == null ? void 0 : _c.some((section) => {
  ...
  const sectionInstructions = ((instructions == null ? void 0 : instructions.sections) ?? []).map((section) => {
  ...
  const prompt = instructions == null ? void 0 : instructions.mainInstruction.text;
  ...
  const dialogTitle = (activeTool == null ? void 0 : activeTool.flyover) ?? translate("toolAssistance.title");
  const tabs = [
  ...
  return reactExports.createElement(
    StatusBarPopover,
    { visible: state.isOpen, onVisibleChange: (visible) => {
      setIsOpen(visible);
    }, closeOnOutsideClick: !state.isPinned, content: reactExports.createElement(
      ...
      }, title: translate(state.isPinned ? "dialog.close" : "toolAssistance.pin") }, state.isPinned ? reactExports.createElement(SvgClose, null) : reactExports.createElement(SvgPin, null)), title: dialogTitle },
      reactExports.createElement(
        "div",
        null,
    ...
    reactExports.createElement(
      ...
      ) : reactExports.createElement(reactExports.Fragment, null), className: cx("uifw-statusFields-toolAssistance-toolAssistanceField", props.className), title: tooltip, style: props.style, labelProps: { className: "prompt" } },
      prompt,
...
function KeyNode({ keyboardKey, className }) {
  return reactExports.createElement(
    "div",
    { className: cx("uifw-toolassistance-key", className) },
