...
function usePanelState(location: StagePanelLocation) {
  ...
  React.useEffect(() => {
    const remove = UiFramework.frontstages.onPanelStateChangedEvent.addListener(
      (e) => {
        if (e.panelDef.location === location) setState(e.panelState);
      }
    );
    return remove;
  }, [location]);
...
function usePanelPinned(location: StagePanelLocation) {
  ...
  React.useEffect(() => {
    return UiFramework.frontstages.onPanelPinnedChangedEvent.addListener(
      (e) => {
        if (e.panelDef.location === location) setPinned(e.pinned);
      }
    );
  }, [location]);
  return pinned;
...
function PanelInfo({ location }: { location: StagePanelLocation }) {
...
function PanelSelect({
  ...
  const [options] = React.useState<Array<SelectOption<string>>>([
    {
      label: StagePanelLocation[StagePanelLocation.Left],
      value: StagePanelLocation[StagePanelLocation.Left],
    },
    {
      label: StagePanelLocation[StagePanelLocation.Top],
      value: StagePanelLocation[StagePanelLocation.Top],
    },
    {
      label: StagePanelLocation[StagePanelLocation.Right],
      value: StagePanelLocation[StagePanelLocation.Right],
    },
    {
      label: StagePanelLocation[StagePanelLocation.Bottom],
      value: StagePanelLocation[StagePanelLocation.Bottom],
    },
  ]);
  return (
    <Select
      ...
      onChange={(value) => {
        const newLocation =
          StagePanelLocation[value as keyof typeof StagePanelLocation];
...
function SelectPanelInfo() {
  ...
  return (
    <>
      <PanelSelect location={location} onChange={(l) => setLocation(l)} />
      <b> panel</b>
...
function SelectWidgetInfo() {
  ...
  return (
    <>
      <WidgetSelect id={id} onChange={(i) => setId(i)} />
      <b> widget</b>
      <WidgetInfo id={id} />
    </>
  );
...
function useWidgetState(id: string) {
  ...
  React.useEffect(() => {
    const remove =
      UiFramework.frontstages.onWidgetStateChangedEvent.addListener((e) => {
        if (e.widgetDef.id === id) setState(e.widgetState);
      });
    return remove;
  }, [id]);
  return state;
...
function WidgetInfo({ id }: { id: string }) {
  ...
  const [isPopout, setIsPopout] = React.useState(
    frontstageDef ? frontstageDef.isPopoutWidget(id) : false
  );
  React.useEffect(() => {
  ...
  const handleXChanged = React.useCallback((value: number | undefined) => {
    if (undefined !== value) setXPos(value);
  }, []);

  const handleYChanged = React.useCallback((value: number | undefined) => {
    if (undefined !== value) setYPos(value);
  }, []);
  ...
  return (
    <>
      ...
      {
        <div>
          Location={isFloating ? "Floating" : isPopout ? "Pop-out" : "Panel"}
        </div>
      }
      <div style={{ display: "flex", alignItems: "center" }}>
        ...
        <Button disabled={isFloating} onClick={handleFloatClick}>
          Float
        </Button>
        <Button
          disabled={isPopout || !widgetDef?.canPopout}
          onClick={handlePopoutClick}
        >
          Pop Out
        </Button>
      </div>
      <Button disabled={!(isFloating || isPopout)} onClick={handleDockClick}>
        Dock
      </Button>
    </>
  );
...
function FrontstageControls() {
  ...
  const newMode: ApplicationMode =
    applicationLayout?.mode === "app" ? "portal" : "app";
  return (
    <>
...
function PanelStateSelect({
  ...
  const [options] = React.useState<Array<SelectOption<string>>>([
    {
      label: StagePanelState[StagePanelState.Open],
      value: StagePanelState[StagePanelState.Open],
    },
    {
      label: StagePanelState[StagePanelState.Minimized],
      value: StagePanelState[StagePanelState.Minimized],
    },
    {
      label: StagePanelState[StagePanelState.Off],
      value: StagePanelState[StagePanelState.Off],
    },
  ]);
  return (
    <Select
...
function WidgetStateSelect({
  ...
  const [options] = React.useState([
    {
      label: WidgetState[WidgetState.Open],
      value: WidgetState[WidgetState.Open],
    },
    {
      label: WidgetState[WidgetState.Closed],
      value: WidgetState[WidgetState.Closed],
    },
    {
      label: WidgetState[WidgetState.Floating],
      value: WidgetState[WidgetState.Floating],
    },
    {
      label: WidgetState[WidgetState.Hidden],
      value: WidgetState[WidgetState.Hidden],
    },
    {
      label: WidgetState[WidgetState.Unloaded],
      value: WidgetState[WidgetState.Unloaded],
    },
  ]);
  return (
    <Select
...
function PanelControls({ location }: { location: StagePanelLocation }) {
  const [state, setState] = React.useState<StagePanelState | undefined>(
    undefined
  );
...
export function FloatingLayoutInfo() {
  const frontstageDef = useActiveFrontstageDef();
  const [floatingIds, setFloatingIds] = React.useState(() =>
    frontstageDef ? frontstageDef.getFloatingWidgetContainerIds() : []
  );
  const [floatingWidgetId, setFloatingWidgetId] = React.useState<
    string | undefined
  >(floatingIds?.length ? floatingIds[0] : undefined);
  const [bounds, setBounds] = React.useState<RectangleProps>(() =>
  ...
  React.useEffect(() => {
    return InternalFrontstageManager.onFrontstageNineZoneStateChangedEvent.addListener(
      (e) => {
        ...
        setTimeout(() => {
          setBounds((prev) => {
            const newBounds = getFloatingWidgetContainerBounds(
              frontstageDef,
              widgetId
            );
  ...
  const handleWidgetIdChanged = React.useCallback(
    (widgetId: string) => {
      setFloatingWidgetId(widgetId);
  ...
  const handleBoundsChanged = React.useCallback(
    (side: "left" | "top" | "bottom" | "right", value: number) => {
      if (floatingWidgetId && frontstageDef) {
        ...
        // setBounds(newBounds);
        frontstageDef.setFloatingWidgetContainerBounds(
          floatingWidgetId,
          newBounds
        );
      }
  ...
  const [widgetIdToLocate, setWidgetIdToLocate] = React.useState<
    string | undefined
  >();
  const handleSetWidgetIdToLocate = React.useCallback(
  ...
  const handleLookup = React.useCallback(() => {
    if (widgetIdToLocate && frontstageDef) {
      const containerId =
        frontstageDef.getFloatingWidgetContainerIdByWidgetId(widgetIdToLocate);
      const briefMessage = containerId
        ? `ContainerId='${containerId}' for WidgetId='${widgetIdToLocate}'`
    ...
    <WidgetContent>
      ...
      >
        ...
        <NumberInput
          ...
          value={bounds.bottom}
          step={5}
          onChange={(value) => handleBoundsChanged("bottom", value ?? 0)}
        />
        <Input
          placeholder="Enter Widget Id to find"
          onChange={handleSetWidgetIdToLocate}
          defaultValue={widgetIdToLocate}
          size="small"
        />
        <Button
          styleType="high-visibility"
          style={{ width: "160px" }}
          disabled={!widgetIdToLocate}
