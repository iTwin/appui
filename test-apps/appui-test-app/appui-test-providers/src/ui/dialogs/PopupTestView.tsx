  ...
  return (
    <div ref={divRef} style={{display:"grid", gridTemplateRows:"auto 1fr", height:"100%", position:"relative", minWidth:"400px", minHeight:"300px"}} >
      {!!showViewPicker && initialViewState &&
        <div><ViewDefinitionSelector imodel={initialViewState.iModel} selectedViewDefinition={initialViewState.id} onViewDefinitionSelected={onViewDefinitionChanged} /></div>}
      {initialViewState &&
        <div><FloatingViewportContent contentId={contentId} initialViewState={initialViewState} onContextMenu={handleContextMenu} /></div>}
    </div>
  );
}
