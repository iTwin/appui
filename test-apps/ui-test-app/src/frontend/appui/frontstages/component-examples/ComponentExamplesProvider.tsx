...
import {
  ...
  ContextMenuInPopup,
  DropdownMenuInPopup,
  GlobalContextMenuInPopup,
  GlobalItwinContextMenuInPopup,
  PopupContextMenuInPopup,
  SamplePopupContextMenu,
} from "./SamplePopupContextMenu";
import { FormatPopupButton } from "./FormatPopupButton";
import { AccudrawSettingsPageComponent } from "../Settings";
import {
  CurrentDateMarkedCustomIconSampleTimeline,
  CurrentDateMarkedSampleTimeline,
  ItemsAppendedSampleTimeline,
  ItemsPrefixedSampleTimeline,
  ItemsReplacedSampleTimeline,
  LocalizedTimeSampleTimeline,
  NoLocalizedTimeSampleTimeline,
  NoRepeatSampleTimeline,
} from "./SampleTimelineComponent";
  ...
  const tabs: SettingsTabEntry[] = [
    {
      ...
      label: "Quantity",
      tooltip: "Quantity Format Settings",
      icon: "icon-measure",
      page: (
        <QuantityFormatSettingsPage
...
function NumericFormatPopup({
      ...
      async function fetchFormatSpec(formatProps: FormatProps) {
        ...
        if (formatterSpec) {
          ...
          if (pu) {
            ...
            const newSpec = await FormatterSpec.create(
              actualFormat.name,
              actualFormat,
              unitsProvider,
              pu
  ...
  React.useEffect(() => {
    async function fetchInitialFormatSpec() {
      ...
      if (pu) {
        const newSpec = await provideFormatSpec(
          initialFormatProps,
          pu,
          unitsProvider
        );
    ...
    if (undefined === formatterSpec) fetchInitialFormatSpec(); // eslint-disable-line @typescript-eslint/no-floating-promises
  }, [
    formatterSpec,
    persistenceUnitName,
    initialMagnitude,
...
function NestedPopup({
  ...
  return (
    <div>
      <button onClick={togglePopup} ref={buttonRef}>
...
export function WeightPickerHost(props: {
  activeWeight: number;
  onLineWeightPick: (weight: number) => void;
}) {
  const { onLineWeightPick, activeWeight } = props;
  const [currentWeight, setCurrentWeight] = React.useState(activeWeight);

  const handleWeightPick = React.useCallback(
    (weight: number) => {
...
export function ColorPickerToggle({ hideRgb }: { hideRgb?: boolean }) {
    ...
    (newColor: ColorDef, e: React.MouseEvent<Element, MouseEvent>) => {
      ...
      UiFramework.dialogs.modal.open(
        <ColorPickerDialog
          ...
          color={newColor}
          colorPresets={presetColors.current}
  ...
  return (
    <ColorSwatch
      className="map-manager-base-item-color"
      colorDef={selectedColor}
      round={false}
      onColorPick={handleBgColorClick}
    />
  );
}
...
export class ComponentExamplesProvider {
  ...
  private static get colorSamples(): ComponentExampleCategory {
    ...
    return {
      ...
      examples: [
        ...
        createComponentExample(
          "Color Picker Dialog no rgb",
          undefined,
          <ColorPickerToggle hideRgb />
        ),
        createComponentExample(
          "Color Picker Popup",
          undefined,
          <ColorPickerPopup initialColor={colorDef} onClose={onPopupClose} />
        ),
        createComponentExample(
          "Color Picker Popup",
          "with Caret",
  ...
  private static get datePickerSample(): ComponentExampleCategory {
    ...
    const monthLetterFormatter = new Intl.DateTimeFormat("default", {
      month: "narrow",
      timeZone: "UTC",
    } as any);
    const projectDate = dateFormatter.format(
      new Date(projectSunriseMs - tzOffsetMs)
    );
    const projectSunriseTime = timeFormatter.format(
      new Date(projectSunriseMs - tzOffsetMs)
    );
    const projectSunsetTime = timeFormatter.format(
      new Date(projectSunsetMs - tzOffsetMs)
    );
    const projectSunTime = timeFormatter.format(
      new Date(projectSunTimeMs - tzOffsetMs)
    );
    const month = monthLetterFormatter.format(
      new Date(projectSunriseMs - tzOffsetMs)
    );
    ...
    return {
      ...
      examples: [
        ...
        createComponentExample(
          "Date Picker Popup w/input",
          undefined,
          <DatePickerHost selected={new Date()} displayEditField={true} />
        ),
        createComponentExample(
          "Date Picker Popup w/input & 12h time",
          undefined,
          <DatePickerHost
            selected={new Date()}
        ...
        createComponentExample(
          "Date Picker Popup w/input & 24h time",
          undefined,
          <DatePickerHost
            selected={new Date()}
            displayEditField={true}
        ...
        createComponentExample(
          "Date Picker Popup w/London date & time",
          undefined,
          <DatePickerHost
            selected={londonDate}
            displayEditField={true}
        ...
        createComponentExample(
          "Date Picker Popup w/LA date & time",
          undefined,
          <DatePickerHost
            selected={laDate}
            displayEditField={true}
        ...
        createComponentExample(
          "Date Picker Popup w/custom formatter",
          undefined,
          <DatePickerHost
            selected={new Date()}
            displayEditField={true}
        ...
        createComponentExample(
          "Date Picker Popup w/IntlFormatter",
          undefined,
          <DatePickerHost
            fieldStyle={{ width: "16em" }}
            selected={new Date()}
            displayEditField={true}
            timeDisplay={TimeDisplay.H12MSC}
            dateFormatter={new IntlFormatter()}
          />
        ),
        createComponentExample(
          "Date Picker Popup w/MDY Formatter",
          undefined,
          <DatePickerHost
            selected={new Date()}
            displayEditField={true}
    ...
    return {
      ...
      examples: [
        createComponentExample(
          ...
          undefined,
          <UnderlinedButton
            onActivate={() => SampleContextMenu.showContextMenu()}
          >
            {" "}
            Open ContextMenu
          </UnderlinedButton>
        ...
        createComponentExample(
          "iTwinUI DropdownMenu",
          "similar to ContextMenu",
          <ButtonWithDropdownMenu />
        ),
        createComponentExample(
          "ContextMenu in Popup",
          undefined,
          <ContextMenuInPopup />
        ),
        createComponentExample(
          "iTwinUi DropdownMenu in Popup",
          "similar to ContextMenu in Popup",
          <DropdownMenuInPopup />
        ),
        createComponentExample(
          "Popup ContextMenu",
        ...
        createComponentExample(
          "iTwinUI Menu at cursor",
          "similar to GlobalContextMenu",
          <GlobalItwinContextMenuInPopup />
        ),
      ],
    };
  ...
  private static get expandableListBlockSamples(): ComponentExampleCategory {
    return {
      ...
      examples: [
        createComponentExample(
          "ExpandableList",
          "ExpandableList with one ExpandableBlock",
        ...
        createComponentExample(
          "ExpandableList",
          "ExpandableList with 3 ExpandableBlocks",
          <ExpandableList className="uicore-full-width">
            <SampleExpandableBlock
              title="Test1"
              isExpanded={false}
        ...
        createComponentExample(
          "ExpandableList w/ singleExpandOnly",
          "ExpandableList with singleExpandOnly prop",
          <ExpandableList
            className="uicore-full-width"
        ...
        createComponentExample(
          "ExpandableList w/ singleIsCollapsible",
          "ExpandableList with singleIsCollapsible prop",
          <ExpandableList
            className="uicore-full-width"
            singleExpandOnly={true}
  ...
  private static get loadingSamples(): ComponentExampleCategory {
    return {
      ...
      examples: [
        createComponentExample(
          "X-Small LoadingSpinner",
          undefined,
          <LoadingSpinner
            size="x-small"
            message="This is a X-Small LoadingSpinner"
          />
        ),
        createComponentExample(
          ...
          <LoadingSpinner
            size="small"
            message="This is a Small LoadingSpinner"
          />
        ),
        createComponentExample(
          "Medium LoadingSpinner",
          undefined,
          <LoadingSpinner size="" message="This is a Medium LoadingSpinner" />
        ),
        createComponentExample(
          ...
          <LoadingSpinner
            size="large"
            message="This is a Large LoadingSpinner"
          />
        ),
        createComponentExample(
          "LoadingStatus",
          undefined,
          <LoadingStatus message="Loading status..." percent={50} />
        ),
        createComponentExample(
          "Basic LoadingPrompt",
          undefined,
        ...
        createComponentExample(
          "LoadingPrompt with message",
          undefined,
          <LoadingPrompt title="Title" message="This is the message" />
        ),
        createComponentExample(
          "LoadingPrompt with Indeterminate Bar",
          undefined,
          <LoadingPrompt
            style={{ width: "100%" }}
            title="Title"
        ...
        createComponentExample(
          "Determinate LoadingPrompt with percent",
          undefined,
          <LoadingPrompt
            title="Title"
            message="This is the message"
        ...
        createComponentExample(
          "Determinate LoadingPrompt with cancel",
          undefined,
          <LoadingPrompt
            title="Title"
            message="This is the message"
        ...
        createComponentExample(
          "Determinate LoadingPrompt with status",
          undefined,
          <LoadingPrompt
            title="Title"
            message="This is the message"
  ...
  private static get _reactMessage(): ReactMessage {
    const reactNode = (
      <span>
        For more details,{" "}
        <UnderlinedButton onClick={() => {}}>click here</UnderlinedButton>.
      </span>
    );
    return { reactNode };
  }
  ...
  private static _activityTool = async () => {
    ...
    while (!isCancelled && progress <= 100) {
      IModelApp.notifications.outputActivityMessage(
        "This is a sample activity message",
        progress
      );
    ...
    const endReason = isCancelled
      ? ActivityMessageEndReason.Cancelled
    ...
    return {
      ...
      examples: [
        ...
        createComponentExample(
          "Activity",
          undefined,
          <UnderlinedButton onActivate={this._activityTool}>
            Activity message
          </UnderlinedButton>
        ),
      ],
  ...
  private static get popupSamples(): ComponentExampleCategory {
    return {
      ...
      examples: [
        createComponentExample(
          "Allow Nested Popup",
          "Remain open when clicking in nested popup",
          <NestedPopup />
        ),
        createComponentExample(
          "Close Nested Popup",
          "Close when clicking in nested popup",
          <NestedPopup closeOnNestedPopupOutsideClick={true} />
        ),
      ],
    };
  }
    ...
    const onTemperatureChange = (value: number) => {
      if (typeof value === "number") {
        IModelApp.notifications.outputMessage(
          new NotifyMessageDetails(
            OutputMessagePriority.Info,
            `Temperature value set to ${value} C`
          )
        );
      ...
      examples: [
        createComponentExample(
          ...
          <QuantityInput
            ...
            onQuantityChange={onLengthChange}
          />
        ),
        createComponentExample(
          "Angle",
          ...
          <QuantityInput
            ...
            onQuantityChange={onAngleChange}
          />
        ),
        createComponentExample(
          "Bearing",
        ...
        createComponentExample(
          ...
          <QuantityInput
            ...
            onQuantityChange={onVolumeChange}
          />
        ),
        createComponentExample(
          "Temperature (Custom)",
        ...
        createComponentExample(
          ...
          <QuantityNumberInput
            style={{ width: "140px" }}
            persistenceValue={initialLength}
            step={0.25}
            snap
            quantityType={QuantityType.Length}
        ...
        createComponentExample(
          "Quantity Number Input",
          "Disabled QuantityType.LengthEngineering",
          <QuantityNumberInput
            style={{ width: "140px" }}
            placeholder={"Specify Length"}
            step={0.25}
        ...
        createComponentExample(
          ...
          <QuantityNumberInput
            showTouchButtons
            persistenceValue={initialVolume}
            step={0.5}
            snap
            quantityType={QuantityType.Volume}
        ...
        createComponentExample(
          ...
          <Listbox
            ...
            onKeyDown={(event: React.KeyboardEvent<HTMLUListElement>) =>
              console.log(`item: ${event.currentTarget?.dataset?.value}`)
            }
          >
  ...
  private static get timelineSamples(): ComponentExampleCategory {
    ...
    examples.push(
      createComponentExample(
        "TimelineComponent",
        "With appended menu items",
        <ItemsAppendedSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With prefixed menu items",
        <ItemsPrefixedSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With menu items replaced",
        <ItemsReplacedSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With no repeat option",
        <NoRepeatSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With timezone offset of 0",
        <NoLocalizedTimeSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With no timezone offset specified",
        <LocalizedTimeSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With with today's date marked by the default marker",
        <CurrentDateMarkedSampleTimeline />
      ),
      createComponentExample(
        "TimelineComponent",
        "With with today's date marked by a star",
        <CurrentDateMarkedCustomIconSampleTimeline />
      )
    );
    return {
