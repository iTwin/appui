import { h as forwardRef, j as jsxRuntimeExports, i as Role, c as classnames, r as reactExports, u as useLatestRef, k as useSafeContext, l as useRootNode, m as getOwnerDocument, s as spriteSheetId, p as parseDOM, H as HtmlSanitizerContext, R as React } from "./iframe-CpRh-TYa.js";
import { p as StagePanelState, W as WidgetState } from "./appui-react-BtU_mNFj.js";
import { B as BadgeType, S as SvgPlaceholder } from "./components-react-Dj8XcCyt.js";
...
import placeholderIcon from "./placeholder-DE1aukJC.js";
...
const Icon$1 = forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Role.svg, {
    ...
    ref: forwardedRef,
    children: hrefBase ? /* @__PURE__ */ jsxRuntimeExports.jsx("use", {
      href: toIconHref(hrefBase)
    }) : null
  });
});
...
function useNormalizedHrefBase(rawHref) {
  ...
  const subscribe = reactExports.useCallback((notify) => {
    const ownerDocument_0 = getOwnerDocument(rootNode);
    const spriteSheet = ownerDocument_0?.getElementById(spriteSheetId);
    if (!rawHref || !ownerDocument_0 || !spriteSheet) return () => {
    };
    if (isHttpProtocol(rawHref, ownerDocument_0)) return () => {
    };
    ...
    (async () => {
      try {
        const resourceUrl = new URL(rawHref, ownerDocument_0.baseURI);
  ...
  return reactExports.useSyncExternalStore(subscribe, getClientSnapshot, () => rawHref);
}
function isHttpProtocol(url, ownerDocument) {
  const {
    protocol
  } = new URL(url, ownerDocument.baseURI);
