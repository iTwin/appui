...
async function run(args: CodemodArgs, optionOverrides: Options) {
  ...
  debug &&
    console.log("Running 'jscodeshift' with following configuration:", {
      transformFile,
      paths,
      options,
    });
...
yargs
  ...
  .command({
    ...
    builder: (command) => {
      return command
        ...
        .option("printOptions", {
          description:
            "Options that are passed down to recast's printer. See: https://github.com/benjamn/recast/blob/master/lib/options.ts",
        })
        .option("skipCss", {
  ...
  .example(
    "$0 v4.0.0/layout-react src",
    "Apply a specific transform on `src` path."
  )
  .example(
    `$0 <codemod> <paths...> --printOptions.quote="single"`,
    "Pass additional options to recast's printer."
  )
  .example(
    `$0 <codemod> <paths...> --printOptions='{\\"lineTerminator\\\":\\"\\r\\n\\"}'`,
...
async function runCssCodemod(args: CodemodArgs, transformFile: string) {
  ...
  args.debug &&
    console.log("Running 'PostCSS' with following configuration:", {
      plugins,
      files,
    });
