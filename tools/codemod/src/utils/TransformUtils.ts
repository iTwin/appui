...
export class AttributeMap extends Map<
  string,
  ExpressionKind | "UnhandledExpression"
> {
  public getAttributeExpression(
    name: string,
    errorHandle?: () => void
...
export function isExpressionKind(
  j: JSCodeshift,
  source: ASTNode
): source is ExpressionKind {
  return j(source).isOfType(j.Expression);
}

export function isSpecifiedConfigExpression(
  source: ASTNode,
  name: string
): source is ConfigExpression {
  return (
    source.type === "ObjectExpression" &&
    "configName" in source &&
    getName((source as ConfigExpression).configName) === name
  );
}

export function getName(
  name: JSXIdentifier | Identifier | JSXMemberExpression | MemberExpression
): string | undefined {
...
export function jsxElementToUniFormat(
  j: JSCodeshift,
  element: ASTPath<JSXElement>
): UniFormat {
  const name = element.node.openingElement.name as
    | JSXIdentifier
    | JSXMemberExpression;
  ...
  element.node.openingElement.attributes?.forEach((attr) => {
    if (attr.type === "JSXAttribute") {
      ...
      if (!value) attrMap.set(key, j.identifier(key));
...
export function getObjectProperty(
  ...
  for (const prop of expression.properties) {
    if (prop.type === "Property" || prop.type === "ObjectProperty") {
      if (
        ("name" in prop.key && prop.key.name === propName) ||
...
export function takeFirstArrayExpressionElement(
  j: JSCodeshift,
  expression: ArrayExpression
): ExpressionKind | undefined {
  const elements = expression.elements;
  if (elements.length === 0) return j.identifier("undefined");
...
export function handleConditionalExpression(
  ...
  handle: (expr: ExpressionKind) => ExpressionKind
): ConditionalExpression {
...
export function concatExpressions(
  j: JSCodeshift,
  expression1: ExpressionKind | undefined,
  expression2: ExpressionKind | undefined
): ArrayExpression {
  const result = j.arrayExpression([]);
  if (expression1?.type === "ArrayExpression")
    result.elements.push(...expression1.elements);
  else if (expression1) result.elements.push(j.spreadElement(expression1));
  if (expression2?.type === "ArrayExpression")
    result.elements.push(...expression2.elements);
  else if (expression2) result.elements.push(j.spreadElement(expression2));
