...
type ImportSpecifierCollection = Collection<ImportSpecifier> &
  ImportSpecifierMethods;
...
function importSpecifierPlugin(j: JSCodeshift) {
  ...
  const methods: ImportSpecifierMethods = {
    getLocalName(this: ImportSpecifierCollection) {
      ...
      if (node.local) return node.local.name;
      if (node.imported) return node.imported.name;
...
export function findRootIdentifiers(
  j: JSCodeshift,
