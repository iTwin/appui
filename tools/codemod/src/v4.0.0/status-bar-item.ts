...
export default function transformer(
  ...
  const customItems = root
    .findObjectExpressions("StatusBarCustomItem")
    .concat(
      root
        .findCallExpressions("StatusBarItemUtilities.createCustomItem")
        .getArguments(4, (arg) => arg.type === "ObjectExpression")
    );

  const items = root
    .findObjectExpressions("StatusBarItem")
    .concat(root.findObjectExpressions("StatusBarActionItem"))
    .concat(root.findObjectExpressions("StatusBarLabelItem"))
    .concat(
      root
        .findCallExpressions("StatusBarItemUtilities.createActionItem")
        .getArguments(6, (arg) => arg.type === "ObjectExpression")
    )
    .concat(
      root
        .findCallExpressions("StatusBarItemUtilities.createLabelItem")
        .getArguments(6, (arg) => arg.type === "ObjectExpression")
    )
    .concat(customItems);
