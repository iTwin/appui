...
import { Ut as WidgetState, dn as PreviewFeaturesProvider, t as init_appui_react, zt as StagePanelState } from "./appui-react-p-O3mm3j.js";
	...
	return {
		...
		getWidgets: () => {
			...
			return [...Array(count)].map((_, index) => {
				const id = index + 1;
				return createWidget(id, { defaultState: index < visibleWidgets ? void 0 : WidgetState.Hidden });
			});
		}
	};
}
...
function init_ControlWidgetVisibility() {
	return (init_ControlWidgetVisibility = __esmMin((() => {
		init_appui_react();
		init_AppUiStory();
		init_Utils();
...
function init_ControlWidgetVisibility_stories() {
	return (init_ControlWidgetVisibility_stories = __esmMin((() => {
		init_Decorators();
		init_AppUiStory();
...
//#endregion
init_ControlWidgetVisibility_stories();
