...
describe("ViewportComponent", () => {
  ...
  const vsCloneDescriptorToRestore = Object.getOwnPropertyDescriptor(
  ...
  let viewState = new SpatialViewState(
    viewDefinitionProps,
    imodelMock.object,
    moq.Mock.ofType<CategorySelectorState>().object,
    moq.Mock.ofType<DisplayStyle3dState>().object,
    moq.Mock.ofType<ModelSelectorState>().object
  );
  ...
  const viewDefinitionProps2: SpatialViewDefinitionProps = {
    ...
    id: "id2",
    modelSelectorId: "id2",
    categorySelectorId: "id2",
    displayStyleId: "id2",
    model: "model",
    code: { spec: "spec", scope: "scope" },
  };
  const viewState2 = new SpatialViewState(
    viewDefinitionProps2,
    imodelMock.object,
    moq.Mock.ofType<CategorySelectorState>().object,
    moq.Mock.ofType<DisplayStyle3dState>().object,
    moq.Mock.ofType<ModelSelectorState>().object
  ...
  tentativePointMock
    .setup((x) => x.isActive)
    .returns(() => tentativePointIsActive);
  tentativePointMock
    .setup((x) => x.getPoint)
    .returns(() => () => Point3d.createZero());
    ...
    .returns(
      () =>
        (
          _pickPoint: Point3d,
          _radius: number,
          _allowNonLocatable?: boolean | undefined,
          _out?: Point3d | undefined
        ) =>
          nearestVisibleGeometryPoint
    );
  viewportMock
    .setup((x) => x.getWorldFrustum)
  ...
  describe("drawingViewChangeEvent", () => {
    it("should register drawingViewChangeEvent", async () => {
      ...
      await TestUtils.flushAsyncOperations();
      ViewportComponentEvents.onDrawingViewportChangeEvent.emit({
        origin: or,
        rotation: rot,
        complete: false,
      });
    ...
    it("should register cubeRotationChangeEvent", async () => {
      ...
      render(
        <ViewportComponent
          ...
          viewState={viewState}
          viewManagerOverride={viewManager.object}
          tentativePointOverride={tentativePointMock.object}
          screenViewportOverride={ScreenViewportMock}
        />
      );
      await TestUtils.flushAsyncOperations();
  ...
  describe("standardRotationChangeEvent", () => {
    it("should register standardRotationChangeEvent", async () => {
      ...
      await TestUtils.flushAsyncOperations();
      ViewportComponentEvents.onStandardRotationChangeEvent.emit({
        standardRotation: StandardViewId.Front,
      });
    });
  });
