...
describe("RatioTypeV2", () => {
  ...
  it("should render with default ratio type", async () => {
    ...
    const renderedComponent = render(
      <RatioTypeV2 formatProps={formatProps} onChange={onChange} />
    );

    expect(renderedComponent.getByText("QuantityFormat.labels.ratioTypeLabel"))
      .to.exist;
    expect(
      renderedComponent.getByText("QuantityFormat.ratio-type.n-to-one.label")
    ).to.exist;
  });
  ...
  it("should be disabled when disabled prop is true", async () => {
    ...
    const renderedComponent = render(
      <RatioTypeV2 formatProps={formatProps} onChange={onChange} disabled />
    );
