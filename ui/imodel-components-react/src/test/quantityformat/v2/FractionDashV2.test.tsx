...
describe("FractionDashV2", () => {
	...
	it("should render with unchecked checkbox when FractionDash trait is not set", async () => {
		...
		const renderedComponent = render(<FractionDashV2 formatProps={formatProps} onChange={onChange} />);

		const checkbox = renderedComponent.getByLabelText("QuantityFormat.labels.fractionDashLabel");
	...
	it("should be disabled when disabled prop is true", async () => {
		...
		const renderedComponent = render(<FractionDashV2 formatProps={formatProps} onChange={onChange} disabled={true} />);
