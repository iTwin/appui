			...
			{formatProps.composite?.units
				? formatProps.composite.units.map((value, index) => (
						<UnitDescr
							...
							name={value.name}
							label={value.label ?? ""}
							parentUnitName={index > 0 ? formatProps.composite!.units[index - 1].name : undefined}
							unitsProvider={unitsProvider}
							index={index}
				...
				: persistenceUnit && <UnitDescr key={persistenceUnit.name} name={persistenceUnit.name} label={persistenceUnit.label} unitsProvider={unitsProvider} index={0} onUnitChange={handleUnitChange} onLabelChange={handleUnitLabelChange} />}
			...
			{formatProps.composite?.units && formatProps.composite.units.length > 1 && (
				<div className="format-inline-row">
					<Label className={"uicore-label"} as="div" displayStyle="inline">
						...
						<IconButton className="format-help-tooltip" size="small" styleType="borderless" label={translate("QuantityFormat.labels.compositeSpacerDescription")}>
