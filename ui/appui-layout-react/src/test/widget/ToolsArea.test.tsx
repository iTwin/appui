...
const SizableButton = (props: {small?: boolean}) => <button>{`${props.small}`}</button>;
...
describe("<ToolsArea />", () => {
  ...
  it("renders correctly with app button (enforces 'small=true')", () => {
    ...
    expect(screen.getByRole("button", {name: "true"})).to.satisfy(selectorMatches(".nz-vertical-tool-area > .nz-app-button > button"));
  });
  ...
  it("renders correctly with vertical and horizontal toolbar", () => {
    const { container } = render(<ToolsArea button={<SizableButton />} horizontalToolbar={<div>HorizontalToolbar</div>} verticalToolbar={<div>VerticalToolbar</div>} />);
