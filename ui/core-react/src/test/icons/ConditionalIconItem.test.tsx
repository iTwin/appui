...
describe("ConditionalIconItem", () => {
  ...
  it("should construct with initial value", () => {
    const sut = new ConditionalIconItem(icon1Getter, syncEventIds, defaultIconSpec);
  ...
  it("test static getValue method", () => {
    ...
    expect(ConditionalIconItem.getValue(new ConditionalIconItem(icon1Getter, syncEventIds, defaultIconSpec))).to.be.equal(defaultIconSpec);
    expect(ConditionalIconItem.getValue(new ConditionalIconItem(icon2Getter, syncEventIds, defaultIconSpec))).to.be.equal(defaultIconSpec);
    expect(ConditionalIconItem.getValue(new ConditionalIconItem(icon1Getter, syncEventIds))).to.be.equal(iconSpec1);
    expect(ConditionalIconItem.getValue(new ConditionalIconItem(icon2Getter, syncEventIds))).to.be.equal(iconSpec2);
  });

  it("test static refreshValue method", () => {
    const sut = new ConditionalIconItem(icon1Getter, syncEventIds, defaultIconSpec);
    expect(sut.value).to.be.equal(defaultIconSpec);
    expect(ConditionalIconItem.refreshValue(sut, new Set<string>(["cat"]))).to.be.false;
    expect(sut.value).to.be.equal(defaultIconSpec);
    expect(ConditionalIconItem.refreshValue(sut, new Set<string>(["sync-id-two"]))).to.be.true;
    expect(sut.value).to.be.equal(iconSpec1);
    expect(ConditionalIconItem.refreshValue(undefined, new Set<string>(["cat"]))).to.be.false;
  });
  ...
  it("test static refreshValue method with capitalized ids", () => {
    ...
    expect(ConditionalIconItem.refreshValue(sut, new Set<string>(["cat"]))).to.be.false;
    expect(sut.value).to.be.equal(defaultIconSpec);
    expect(ConditionalIconItem.refreshValue(sut, new Set<string>(["sync-id-three"]))).to.be.true;
    expect(sut.value).to.be.equal(iconSpec1);
    expect(ConditionalIconItem.refreshValue(undefined, new Set<string>(["cat"]))).to.be.false;
  });
});
