...
describe("useCrossOriginPopup", () => {
  ...
  it("should open popup if initial visibility is 'ON'", () => {
    ...
    expect(window.open).to.have.been.called.with.calledOnceWithExactly(
      fakeUrl,
      fakeTitle,
  ...
  it("should close popup when visibility change (ON->OFF)", () => {
    ...
    window.open = sandbox.stub(window, "open").callsFake(() => {
      return fakeWindow;
    });
    const onClosePopup = sinon.spy();
    let visible = true;
  ...
  it("should call the 'onClose' callback when the popup is closed by the end-user", () => {
    ...
    const windowOpenStub = sandbox.stub(window, "open").callsFake(() => {
      return fakeWindow;
    });
