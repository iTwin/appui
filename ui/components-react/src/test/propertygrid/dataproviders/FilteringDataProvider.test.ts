      ...
      records: {
          ...
          TestUtils.createArrayProperty(
            ...
            [
              ...
              TestUtils.createStructProperty(
                "Struct1-2-1-2",
                {
                  "Array1-2-1-2-1": TestUtils.createArrayProperty(
                    "Array1-2-1-2-1",
                    [
                      TestUtils.createPrimitiveStringProperty(
                        "Property1-2-1-2-1-1",
        ...
        Cat2: [
          TestUtils.createStructProperty(
            "Struct2",
            {
              "Property2-1": TestUtils.createPrimitiveStringProperty(
                "Property2-1",
                "V1"
              ),
              "Property2-2": TestUtils.createPrimitiveStringProperty(
                "Property2-2",
                "V1"
              ),
              "Array2-3": TestUtils.createArrayProperty(
                "Array2-3",
                [
                  TestUtils.createPrimitiveStringProperty(
                    "Property2-3-1",
                    "V1"
                  ), // Match, do not expand to node
                ],
                false
              ),
            },
  ...
  describe("getData", () => {
    ...
    it("Should return empty property data and matchesCount equal to 0 if filter is enabled and nothing matches it", async () => {
      ...
      mockFilterer
        ...
        .returns(async () => ({ matchesFilter: false }));
      mockFilterer
        .setup(async (x) =>
          x.categoryMatchesFilter(moq.It.isAny(), moq.It.isAny())
        )
        .returns(async () => ({ matchesFilter: false }));
      ...
      expect(propertyData.description).to.equal(
        originalPropertyData.description
      );
    ...
    function findRecord(
      ...
      for (const record of records) {
        if (record.property.name === name) return record;
    ...
    function findRecordFromPropertyData(
      name: string,
    ...
    function findCategoryFromPropertyData(
      name: string,
      propertyData: PropertyData
    ): PropertyCategory {
      ...
      throw Error(
        `Property category with name: ${name} not found. Check test data.`
      );
    }
    ...
    it("Should return expected filtered data if filter is enabled and there are matches", async () => {
            ...
            TestUtils.createArrayProperty(
              ...
              [
                ...
                TestUtils.createStructProperty(
                  "Struct1-1-1-2",
                  {
                    // Matches, force include descendants, expand to node
                    "Array1-1-1-2-1": TestUtils.createArrayProperty(
                      "Array1-1-1-2-1",
                      [
                        TestUtils.createPrimitiveStringProperty(
                ...
                TestUtils.createStructProperty(
                  "Struct1-2-1-2",
                  {
                    "Array1-2-1-2-1": TestUtils.createArrayProperty(
                      ...
                      [],
                      false
                    ), // Matches, do not include descendants, expand to node
                  },
                  true
                ),
              ],
      ...
      (
        findRecordFromPropertyData("Array1-2-1", originalPropertyData)
          .value as ArrayValue
      ).itemsTypeName = "struct";

      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Struct1-1-1-2", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({
      ...
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property1-1-1-3", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({
          ...
          shouldExpandNodeParents: true,
          matchesCount: 6,
          filteredTypes: [FilteredType.Value, FilteredType.Label],
        }));
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Array1-2-1-2-1", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({
      ...
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property2-1", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({
      ...
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property2-2", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({
      ...
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property2-3-1", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({ matchesFilter: true }));
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(moq.It.isAny(), moq.It.isAny())
        )
      ...
      mockFilterer
        .setup(async (x) =>
          x.categoryMatchesFilter(
            findCategoryFromPropertyData("Cat1-1", originalPropertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({
      ...
      expect(filteredData.description).to.equal(
        expectedFilteredData.description
      );
      expect(filteredData.categories).to.deep.equal(
        expectedFilteredData.categories
      );
      expect(filteredData.records).to.deep.equal(expectedFilteredData.records);
      expect(filteredData.matchesCount).to.deep.equal(28);
      expect(filteredData.filteredTypes).to.deep.equal([
    ...
    it("Should return expected active match when getActiveMatch() is called", async () => {
      ...
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property1", propertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({ matchesFilter: true, matchesCount: 8 }));
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property2", propertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({ matchesFilter: true, matchesCount: 6 }));

      mockFilterer
        .setup(async (x) =>
          x.categoryMatchesFilter(
            findCategoryFromPropertyData("Cat1", propertyData),
            moq.It.isAny()
          )
        )
        .returns(async () => ({ matchesFilter: true, matchesCount: 2 }));
      ...
      expect(activeMatch).to.deep.equal({
        highlightedItemIdentifier: "Property2",
        highlightIndex: 0,
      });
    });
    ...
    it("Should return expected active match when getActiveMatch() is called and only label matches count are provided", async () => {
      ...
      mockFilterer
        ...
        .returns(async () => ({ matchesFilter: true, matchesCount: 3 }));
      mockFilterer
        .setup(async (x) =>
          x.recordMatchesFilter(
            findRecordFromPropertyData("Property2", propertyData),
    ...
    it("Should return expected active match when getActiveMatch() is called and propertyCategory match is returned", async () => {
      ...
      mockFilterer
        ...
        .returns(async () => ({ matchesFilter: true, matchesCount: 5 }));
      ...
      expect(activeMatch).to.deep.equal({
        highlightedItemIdentifier: "Cat1",
        highlightIndex: 3,
      });
    });
