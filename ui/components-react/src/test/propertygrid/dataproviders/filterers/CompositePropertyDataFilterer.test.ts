...
describe("CompositePropertyDataFilterer", () => {
  ...
  const testRecordArray: PropertyRecord[] = [
    TestUtils.createPrimitiveStringProperty("TestParent", "TestParent"),
  ];
  const testCategoryArray: PropertyCategory[] = [
    { name: "Par1", label: "Parent 1", expand: true, childCategories: [] },
  ];
  ...
  beforeEach(() => {
    ...
    leftFilterMock
      ...
      .returns(() => leftOnFilterChanged);
    rightFilterMock
      .setup((x) => x.onFilterChanged)
      .returns(() => rightOnFilterChanged);
  });
  ...
  describe("AND operator", () => {
    ...
    describe("matchesFilter", () => {
      it("Should not match PropertyRecord if both filters do not match", async () => {
        ...
        const matchResult = await compositeFilter.recordMatchesFilter(
          testRecord,
          testRecordArray
        );
      ...
      it("Should not match PropertyCategory if both filters do not match", async () => {
        ...
        const matchResult = await compositeFilter.categoryMatchesFilter(
          testCategory,
          testCategoryArray
        );
        expect(matchResult).to.deep.eq({ matchesFilter: false });
