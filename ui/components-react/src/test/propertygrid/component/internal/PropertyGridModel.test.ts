...
describe("MutablePropertyGridModel", () => {
  const testCases: Array<
    PropertyGridModelTestData & { unexistingItemKey: string }
  > = [
    {
      testName: "Single category",
      propertyData: {
        label: TestUtils.createPrimitiveStringProperty("Label", "Value"),
        categories: [{ name: "Cat1", label: "Cat 1", expand: true }],
        records: {
          Cat1: [],
        },
      },
    ...
    //   },
    //   expectedLastItemData: {
    //     "Cat1_Cat1-1_Cat1-1-1_Prop": {
    //       isLastInRootCategory: true,
    //       lastInNumberOfCategories: 3,
    //     },
    //   },
    //   unexistingItemKey: "does not exist",
    // },
    ...
    //               "Struct1-3-1-2",
    //               {
    //                 "Array1-3-1-2-1": TestUtils.createArrayProperty(
    //                   "Array1-3-1-2-1",
    //                   [
    //                     TestUtils.createPrimitiveStringProperty(
    //                       "Property1-3-1-2-1-1",
    ...
    //               "V1"
    //             ),
    //             "Property2-3": TestUtils.createPrimitiveStringProperty(
    //               "Property2-3",
    //               "V1"
    //             ),
    //           },
    ...
    //   },
    //   expectedLastItemData: {
    //     "Cat1_Cat1-1_Array1-1-1_Property1-1-1-5_4": {
    //       isLastInRootCategory: false,
    //       lastInNumberOfCategories: 1,
    //     },
    //     "Cat1_Cat1-2_Array1-2-1_Property1-2-1-5_4": {
    //       isLastInRootCategory: false,
    //       lastInNumberOfCategories: 1,
    //     },
    //     "Cat1_Cat1-3_Array1-3-1": {
    //       isLastInRootCategory: true,
    //       lastInNumberOfCategories: 2,
    //     },
    //     "Cat2_Cat2-1_Cat2-1-1": {
    ...
    //     },
    //     "Cat2_Cat2-1_Cat2-1-2_Struct": {
    //       isLastInRootCategory: false,
    //       lastInNumberOfCategories: 1,
    //     },
    //     "Cat2_Cat2-1_Cat2-1-3": {
    //       isLastInRootCategory: false,
    //       lastInNumberOfCategories: 1,
    //     },
    //     "Cat2_Cat2-2": {
  ...
  describe("Initialize MutablePropertyGridModel correctly", () => {
    for (const testCase of testCases) {
      it(testCase.testName, () => {
        const createCategorySpy = vi.spyOn(factory, "createGridCategory");
        const createPropertySpy = vi.spyOn(
          factory,
        ...
        const expectedFlatProperties = GridUtils.getFlattenedPropertyData(
          testCase.propertyData
        );

        const expectedCategories = expectedFlatProperties.filter(
          (property) => !(property.item instanceof PropertyRecord)
        );
        const expectedRecords = expectedFlatProperties.filter(
          (property) => property.item instanceof PropertyRecord
        );
  ...
  describe("getFlatGrid", () => {
    for (const testCase of testCases) {
      it(testCase.testName, () => {
        const gridModel = new MutablePropertyGridModel(
          testCase.propertyData,
          factory
        );
        ...
        const expectedFlatGrid = GridUtils.getFlattenedPropertyData(
          testCase.propertyData,
          false
        );
  ...
  function assertGridModel(
    gridModel: MutablePropertyGridModel,
  ...
  describe("getItem", () => {
    for (const testCase of testCases) {
      it(`Throws when getting item that does not exist: ${testCase.testName}`, () => {
        ...
        expect(() => gridModel.getItem(testCase.unexistingItemKey)).to.throw(
          Error,
          `Grid item at provided key not found: ${testCase.unexistingItemKey}`
        );
      });
      ...
      it(`${testCase.testName}`, () => {
        ...
        rootCategories.forEach((category, index) =>
          GridUtils.assertCategoryEquals(
            category,
            testCase.propertyData.categories[index]
          )
        );
      });
    }
