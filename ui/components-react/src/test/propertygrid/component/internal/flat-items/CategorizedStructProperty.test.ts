...
import { MutableCategorizedStructProperty } from "../../../../../components-react/propertygrid/internal/flat-items/MutableCategorizedStructProperty.js";
...
describe("CategorizedStructProperty", () => {
  ...
  describe("Should correctly initialize categorized array property", () => {
    it("Should correctly initialize categorized array property", () => {
      ...
      expect(
        () =>
          new MutableCategorizedStructProperty(
            propertyRecord,
            "Cat1",
            "Cat1",
            -1,
    ...
    it("Should correctly initialize categorized struct property", () => {
      ...
      const property = new MutableCategorizedStructProperty(
        propertyRecord,
        "Cat1",
        "Cat1",
        0,
    ...
    it("Should throw when initializing categorized struct property with primitive record", () => {
      ...
      expect(property).to.throw(
        Error,
        "Record with incorrect value format passed to property: expected Struct, got Primitive"
      );
    });
    ...
    it("Should throw when initializing categorized struct property with array record", () => {
      ...
      expect(property).to.throw(
        Error,
        "Record with incorrect value format passed to property: expected Struct, got Array"
      );
    });
  ...
  describe("isExpanded", () => {
    function testIsExpanded(expectedIsExpanded: boolean) {
      it(`isExpanded should return ${expectedIsExpanded} when autoExpand: ${expectedIsExpanded}`, () => {
        const propertyRecord = TestUtils.createStructProperty(
          "Prop",
          {},
          expectedIsExpanded
        );
