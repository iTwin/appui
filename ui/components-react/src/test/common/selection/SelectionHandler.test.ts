...
describe("SelectionHandler", () => {
  ...
  const itemsSelectedCallback =
    moq.Mock.ofType<OnItemsSelectedCallback<string>>();
  const itemsDeselectedCallback =
    moq.Mock.ofType<OnItemsDeselectedCallback<string>>();
  const itemSelectionHandler1 =
    moq.Mock.ofType<SingleSelectionHandler<string>>();
  const itemSelectionHandler2 =
    moq.Mock.ofType<SingleSelectionHandler<string>>();
  const itemSelectionHandler3 =
    moq.Mock.ofType<SingleSelectionHandler<string>>();
  const componentSelectionHandler =
    moq.Mock.ofType<MultiSelectionHandler<string>>();
  ...
  describe("updateDragAction", () => {
    ...
    it("calls updateSelection", () => {
      ...
      handler.createDragAction(
        ...
        [
          [
            itemSelectionHandler1.object,
            itemSelectionHandler2.object,
            itemSelectionHandler3.object,
          ],
        ],
  ...
  describe("completeDragAction", () => {
      ...
      itemsSelectedCallback.verify(
        (x) =>
          x(
            [
              itemSelectionHandler1.object.item(),
              itemSelectionHandler3.object.item(),
            ],
            false
          ),
        moq.Times.once()
      );
      itemsDeselectedCallback.verify(
        (x) => x([itemSelectionHandler2.object.item()]),
        moq.Times.once()
      );
    });
    ...
    it("handles items selection changing twice during the operation", () => {
      ...
      handler.completeDragAction();
      itemsSelectedCallback.verify(
        (x) => x([itemSelectionHandler1.object.item()], false),
        moq.Times.once()
      );
      itemsDeselectedCallback.verify(
        (x) => x([itemSelectionHandler2.object.item()]),
  ...
  describe("createSelectionFunction", () => {
    ...
    beforeEach(() => {
      selectionFunction = handler.createSelectionFunction(
        componentSelectionHandler.object,
        itemSelectionHandler1.object
      );
    });
    ...
    describe("None", () => {
      it("does nothing", () => {
        ...
        selectionFunction();
        itemsSelectedCallback.verify(
          (x) => x([itemSelectionHandler1.object.item()], true),
          moq.Times.never()
        );
      });
    });
    ...
    describe("SingleAllowDeselect", () => {
      ...
      it("does not replace selection if selectionMode is SingleAllowDeselect and selected item is deselected", () => {
        ...
        itemsDeselectedCallback.verify(
          (x) => x([itemSelectionHandler1.object.item()]),
          moq.Times.once()
        );
        componentSelectionHandler.verify(
          (x) => x.deselectAll(),
    ...
    describe("Extended", () => {
      ...
      it("selects between if shiftDown is true and replaces selection if ctrlDown is false", () => {
        handler.selectionMode = SelectionMode.Extended;
        const selectionFunction2 = handler.createSelectionFunction(
          componentSelectionHandler.object,
          itemSelectionHandler2.object
        );
        componentSelectionHandler
          .setup((x) => x.selectBetween(moq.It.isAny(), moq.It.isAny()))
          .returns((item1, item2) => [item1, item2]);
        selectionFunction();
        ...
        componentSelectionHandler.verify(
          (x) =>
            x.selectBetween(
              itemSelectionHandler1.object.item(),
              itemSelectionHandler2.object.item()
            ),
          moq.Times.once()
        );
        componentSelectionHandler.verify(
      ...
      it("does not select between if shiftDown is false", () => {
        ...
        componentSelectionHandler.verify(
          (x) => x.selectBetween(moq.It.isAny(), moq.It.isAny()),
          moq.Times.never()
        );
        itemSelectionHandler1.verify((x) => x.select(), moq.Times.once());
        ...
        itemsSelectedCallback.verify(
          (x) => x([itemSelectionHandler2.object.item()], true),
          moq.Times.once()
        );
        itemsDeselectedCallback.verify(
          (x) => x(moq.It.isAny()),
      ...
      it("does not change last node interacted with if shiftDown is true", () => {
        ...
        const selectionFunction3 = handler.createSelectionFunction(
          componentSelectionHandler.object,
          itemSelectionHandler3.object
        );
    ...
    describe("Multiple", () => {
      it("does not call callback if drag operation is in progress", () => {
        ...
        itemsSelectedCallback.verify(
          (x) => x([itemSelectionHandler3.object.item()], false),
          moq.Times.once()
        );
        itemsDeselectedCallback.verify(
          (x) => x([itemSelectionHandler2.object.item()]),
...
describe("DragAction", () => {
  ...
  const createItemHandlers = (
    ...
    columnCount: number,
    isSelected?: boolean
  ): Array<Array<SingleSelectionHandler<Item>>> => {
    const itemHandlers: Array<Array<SingleSelectionHandler<Item>>> = [];
    for (let row = 0; row < rowCount; row++) {
      itemHandlers[row] = [];
      for (let column = 0; column < columnCount; column++) {
        itemHandlers[row][column] = createItemHandler(
          { row, column },
          isSelected
        );
      }
  ...
  const componentSelectionHandler: MultiSelectionHandler<Item> = {
    ...
    selectBetween: () => [],
    updateSelection: () => {},
    areEqual: (item1: Item, item2: Item) =>
  ...
  describe("updateDragAction", () => {
    it("returns first item in deselections if isSelected returns true", () => {
      const itemHandlers = [
        [
          createItemHandler({ row: 0, column: 0 }, true),
          createItemHandler({ row: 0, column: 1 }),
        ],
      ];
      ...
      expect(
        findItem(selectionChanges.deselections, itemHandlers[0][0].item())
      ).toBeTruthy();
    });
    ...
    it("returns first item in selections if isSelected returns false", () => {
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[0][0].item())
      ).toBeTruthy();
    });
    ...
    it("returns items in selections or deselections based on isSelected result", () => {
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[0][2].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.deselections, itemHandlers[0][1].item())
      ).toBeTruthy();
    });
    ...
    it("returns selection changes from left to right", () => {
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[0][3].item())
      ).toBeTruthy();
      ...
      expect(selectionChanges.selections.length).toEqual(1);
      expect(
        findItem(selectionChanges.selections, itemHandlers[0][4].item())
      ).toBeTruthy();
      ...
      expect(selectionChanges.selections.length).toEqual(2);
      expect(
        findItem(selectionChanges.selections, itemHandlers[0][1].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[0][3].item())
      ).toBeTruthy();
    ...
    it("returns selection changes from top to bottom", () => {
      ...
      expect(selectionChanges.selections.length).toEqual(2);
      expect(
        findItem(selectionChanges.selections, itemHandlers[2][0].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[3][0].item())
      ).toBeTruthy();
      ...
      expect(selectionChanges.selections.length).toEqual(1);
      expect(
        findItem(selectionChanges.selections, itemHandlers[4][0].item())
      ).toBeTruthy();
      ...
      expect(selectionChanges.selections.length).toEqual(2);
      expect(
        findItem(selectionChanges.selections, itemHandlers[1][0].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[3][0].item())
      ).toBeTruthy();
    ...
    it("returns selection changes from bottom left to top right", () => {
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[2][1].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[2][2].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[3][1].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[3][2].item())
      ).toBeTruthy();
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[1][1].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[1][2].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[1][3].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[2][3].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[3][3].item())
      ).toBeTruthy();
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[4][1].item())
      ).toBeTruthy();
    });
    ...
    it("returns selection changes from top right to bottom left", () => {
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[1][4].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[2][2].item())
      ).toBeTruthy();
    ...
    it("returns selection changes from bottom right to top left", () => {
      ...
      expect(
        findItem(selectionChanges.selections, itemHandlers[3][4].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[4][3].item())
      ).toBeTruthy();
      expect(
        findItem(selectionChanges.selections, itemHandlers[4][4].item())
      ).toBeTruthy();
    });
  });
});
