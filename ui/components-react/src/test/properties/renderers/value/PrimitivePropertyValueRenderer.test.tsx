...
class AsyncValuesTypeConverter extends TypeConverter {
  public sortCompare(
    _lhs: Primitives.Value,
    _rhs: Primitives.Value,
    _ignoreCase?: boolean
  ) {
    return 0;
  }
...
describe("PrimitivePropertyValueRenderer", () => {
  describe("render", () => {
    ...
    it("renders async value with default value in context", async () => {
      ...
      const propertyRecord = TestUtils.createPropertyRecord(
        value,
        { key: "test", label: "test" },
        "async"
      );
      ...
      const renderedElement = render(
        <>{renderer.render(propertyRecord, context)}</>
      );
