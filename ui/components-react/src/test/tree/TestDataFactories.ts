...
export class TestTreeDataProvider implements ITreeDataProvider {
  private _delayedLoads: {
    [nodeId: string]: ResolvablePromise<DelayLoadedTreeNodeItem>;
  } = {};
  ...
  public getNodesCount = async (parent?: TreeNodeItem): Promise<number> =>
    this._getChildren(parent).length;
  ...
  public getNodes = async (
    ...
    const children = this._getChildren(parent).slice(
      page.start,
  ...
  private _makeTreeNodeItem = (
    node: TestTreeHierarchyNode
  ): DelayLoadedTreeNodeItem => {
    return {
      id: node.id,
      label: PropertyRecord.fromString(node.id, "label"),
  ...
  private _findNode = (nodeId: string): TestTreeHierarchyNode => {
    const findNodeRecursively = (
      children: TestTreeHierarchyNode[]
    ): TestTreeHierarchyNode | undefined => {
    ...
    if (!node) throw new Error(`Unknown tree nodeId: '${nodeId}'`);

    return node;
  };

  private _getChildren = (parent?: TreeNodeItem): TestTreeHierarchyNode[] => {
    if (!parent) return this._hierarchy;
