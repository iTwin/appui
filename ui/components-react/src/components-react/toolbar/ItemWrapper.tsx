...
export function ItemWrapper(props: ItemWrapperProps) {
  const { expandsTo, direction, overflowExpandsTo, overflowDirection } =
    useToolbarWithOverflowDirectionContext();
  const { hasOverflow, onResize, useHeight } = useToolItemEntryContext();
  const ref = useResizeObserverSingleDimension<HTMLDivElement>(
    onResize,
    useHeight
  );
...
/** @internal */
export function useResizeObserverSingleDimension<T extends Element>(
  onResize: (size: number) => void,
  useHeight: boolean
) {
  const handleResize = React.useCallback(
    (w: number, h: number) => {
      if (useHeight) return onResize(h);
      return onResize(w);
    },
    [useHeight, onResize]
  );
