...
describe("BasicToolWidget", () => {
  beforeEach(() => {
    sinon.stub(Element.prototype, "getBoundingClientRect").callsFake(function rect(this: any) {
      if(this instanceof HTMLButtonElement || this.firstElementChild?.tagName === "BUTTON") {
  ...
  it("BasicToolWidget with bentley B should render", () => {
    const { container } = render(<BasicToolWidget icon={"icon-bentley-systems"} />);
    expect(container).to.satisfy(childStructure([
  ...
  it("BasicToolWidget with Categories and Models should render", () => {
    ...
    const { container } = render(<BasicToolWidget showCategoryAndModelsContextTools={true} />);
  ...
  it("BasicToolWidget with suffix and prefix items should render correctly", () => {
    const { container } = render(<BasicToolWidget additionalVerticalItems={ToolbarHelper.createToolbarItemsFromItemDefs([testV1Def, testV2Def])}
  ...
  it("BasicToolWidget should refresh when props change", () => {
    const { container, rerender } = render(<BasicToolWidget additionalVerticalItems={ToolbarHelper.createToolbarItemsFromItemDefs([testV1Def, testV2Def])}
      additionalHorizontalItems={ToolbarHelper.createToolbarItemsFromItemDefs([testH1Def, testH2Def])} />);
    expect(container).to.satisfy(childStructure([
      `.components-items.components-vertical .components-toolbar-item-container:nth-child(1) [data-item-id='Select']`,
      `.components-items.components-vertical .components-toolbar-item-container:nth-child(2) [data-item-id='test-v1-tool']`,
    ...
    rerender(<BasicToolWidget additionalVerticalItems={undefined}
