 ...
 */
export class ToolAssistanceWidgetContent extends React.Component<
...
> {
  ...
  constructor(p: ToolAssistanceFieldProps) {
    ...
    this._showPromptAtCursorSetting = new UiStateEntry(
      ToolAssistanceWidgetContent._toolAssistanceKey,
      ToolAssistanceWidgetContent._showPromptAtCursorKey,
      () => this.state.showPromptAtCursor
    );
    // eslint-disable-next-line deprecation/deprecation
    this._mouseTouchTabIndexSetting = new UiStateEntry(
      ToolAssistanceWidgetContent._toolAssistanceKey,
      ToolAssistanceWidgetContent._mouseTouchTabIndexKey,
        ...
        <div>
          ...
          <div className="uifw-toolAssistance-content">
            <NZ_ToolAssistanceInstruction
              key="main"
              image={ToolAssistanceWidgetContent.getInstructionImage(
    ...
    if (
      ...
      if (
        ...
        image = (
          // eslint-disable-next-line deprecation/deprecation
          <FillCentered>
            {ToolAssistanceWidgetContent.getKeyNode(
              key,
              0,
      ...
      if (instruction.keyboardInfo) {
        image = ToolAssistanceWidgetContent.getInstructionKeyboardImage(
          instruction.keyboardInfo
        );
      } else {
        Logger.logError(
        ...
        <div className="uifw-toolassistance-key-group">
          <span className="row1">
            {keyboardInfo.keys.map((key: string, index1: number) => {
              return ToolAssistanceWidgetContent.getKeyNode(
                key,
                index1,
      ...
      image = (
        ...
        <FillCentered>
          {keyboardInfo.keys.map((key: string, index3: number) => {
            ...
            return ToolAssistanceWidgetContent.getKeyNode(key, index3, className);
          })}
        </FillCentered>
      );
    } else if (keyboardInfo.keys[0]) {
      if (keyboardInfo.keys[0].length > 1)
        image = ToolAssistanceWidgetContent.getKeyNode(
          keyboardInfo.keys[0],
          0,
          "uifw-toolassistance-key-large"
        );
      else image = ToolAssistanceWidgetContent.getKeyNode(keyboardInfo.keys[0], 0);
    } else {
      Logger.logError(
        UiFramework.loggerCategory("ToolAssistanceField"),
