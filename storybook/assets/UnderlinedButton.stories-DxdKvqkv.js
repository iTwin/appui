...
function UnderlinedButton(props) {
	const handleKeyUp = import_react.useCallback((event) => {
		switch (event.key) {
			...
			case " ": props.onActivate && props.onActivate();
		}
	}, [props]);
	...
	const className = (0, import_classnames.default)("core-underlined-button", props.className ? props.className : void 0);
...
function init_UnderlinedButton$1() {
	return (init_UnderlinedButton$1 = __esmMin((() => {
...
function init_UnderlinedButton_stories() {
	return (init_UnderlinedButton_stories = __esmMin((() => {
		init_Decorators();
