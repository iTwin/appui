...
import { S as ConditionalStringValue, r as init_appui_abstract, t as require_Key_enum } from "./Key.enum-1tPeVDEH.js";
import { i as init_ConditionalIconItem, n as init_IconComponent, r as ConditionalIconItem, t as Icon } from "./IconComponent-BoZHJngj.js";
import { n as init_Orientation, t as Orientation } from "./Orientation-BX1mFWWJ.js";
//#region ../../ui/core-react/src/core-react/focus/ItemKeyboardNavigator.tsx
var import_Key_enum, ItemKeyboardNavigator;
function init_ItemKeyboardNavigator() {
	return (init_ItemKeyboardNavigator = __esmMin((() => {
		...
		ItemKeyboardNavigator = class {
			...
			handleKeyUpEvent(event, index) {
				switch (event.key) {
					...
					case import_Key_enum.Key.ArrowRight.valueOf(): this.determineOrientation(event, index);
				}
			}
			focusFirstItem() {
				this.onFocusItem(0);
			...
			*/
			switchItemOnArrowPress(event, index) {
				...
				if (targetDirection) {
					...
					else if (this._allowWrap) {
						if (pressed === import_Key_enum.Key.ArrowLeft.valueOf() || pressed === import_Key_enum.Key.ArrowUp.valueOf()) this.focusLastItem();
...
function init_IconHelper() {
	return (init_IconHelper = __esmMin((() => {
